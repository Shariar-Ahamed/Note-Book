...
def parse_section_body(body_md, sec_num=""):
    ...
    while i < n:
        ...
        while i < n:
            l = lines[i]
            s = l.strip()
            if not s or s == '---' or s.startswith('```') or (s.startswith('|') and s.endswith('|')) or s.startswith('### ') or s.startswith('## ') or s.startswith('>') or s.startswith('* ') or s.startswith('- '):
                break
            para_lines.append(s)
            i += 1
        if para_lines:
    ...
    for b in blocks:
        ...
        if b_type == 'subheading':
            ...
            elif 'real-life example' in sub.lower():
            ...
            elif 'logic' in sub.lower():
                out_html.append('<span class="tag-label purple">Logic Trace</span>')
            elif 'কীভাবে কাজ করে' in sub:
                out_html.append('<span class="tag-label cyan">কীভাবে কাজ করে?</span>')
            elif 'structure' in sub.lower():
                out_html.append('<span class="tag-label purple">Structure</span>')
            else:
            ...
            if lang == 'javascript' or (not lang and ('for ' in code_str or 'let ' in code_str or 'const ' in code_str or 'console.log' in code_str)):
                top_label = "JavaScript"
                sub_title = "Example"
                if last_subheading and last_subheading.lower() not in ('output', 'output:'):
  ...
  <pre>{highlight_js(code_str)}</pre>
                ...
                is_output = ('output' in last_subheading.lower()) or (code_str.strip() in ('Product found.', 'Product found!', 'ReferenceError', '78', '10', '15', '4', '5050', '89'))
                has_diagram_chars = any(c in code_str for c in ('↓', '→', '┌', '└', '├', '│', '┴', '┬', '*****', '***')) or ('<=' in code_str and 'true' in code_str)
  ...
  <pre>{html.escape(code_str)}</pre>
            ...
            out_html.append(f'<div class="def-box english" style="margin: 6px 0;"><strong>Teacher:</strong> {inline_format(b[1])}</div>')
            ...
            p_text = b[1]
            if p_text.startswith('**Loop** হলো') or p_text.startswith('**`for` loop** হলো') or p_text.startswith('**`while` loop** হলো') or p_text.startswith('**`break`** হলো'):
            ...
            elif p_text.startswith('**ভুল:**') or p_text.startswith('**সতর্কতা:**') or p_text.startswith('**Warning:**'):
...
all_secs = numbered_secs + [sec52_md]

# Part definitions: (trigger_sec_num, banner_text)
PART_TRIGGERS = {
    1: "Part 01 — Core 'for' Loop & Numeric Iteration (5.1 – 5.12)",
    13: "Part 02 — 'while', 'do...while' & Infinite Loop Traps (5.13 – 5.20)",
    21: "Part 03 — Loop Flow Control: 'break' & 'continue' (5.21 – 5.24)",
    25: "Part 04 — Modern Iterators: 'for...of' & 'for...in' (5.25 – 5.32)",
    33: "Part 05 — Nested Loops, Patterns & Real Data Processing (5.33 – 5.43)",
    44: "Part 06 — Scope, Advanced Mechanics & Practical Mini-Projects (5.44 – 5.52)",
}
...
<head>
  ...
  <title>JS Master Study Documentation — Chapter 05: Loops &amp; Iteration</title>
...
<body>
  ...
  <div class="doc-page">
    ...
    <header class="master-banner">
      ...
      <div class="banner-sub">CHAPTER 05: LOOPS &amp; ITERATION</div>
      <div class="meta-grid">
        <div class="meta-item">
          <div class="meta-label">Total Sections</div>
          <div class="meta-val">52 Modules + Practice</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Curriculum Part</div>
          <div class="meta-val">Part 02 — Core Mechanics (1/3)</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Core Mechanics</div>
          <div class="meta-val">Iteration, Iterables &amp; Scope</div>
        </div>
        <div class="meta-item">
    ...
    <div class="study-card" style="border-left: 4px solid var(--blue-accent); background: #f8fafc; padding: 12px 16px; margin-bottom: 12px;">
      <h2 style="font-size: 13.5px; color: var(--navy-mid); margin-bottom: 6px;">Loops &amp; Iteration — একই কাজ বারবার করার Smart উপায়</h2>
      <p class="text-p">JavaScript-এ অনেক সময় আমাদের একই কাজ <strong>বারবার</strong> করতে হয়। যেমন:</p>
      <ul class="clean-list">
        <li>1 থেকে 100 পর্যন্ত number print করা</li>
        <li>100 জন student-এর নাম দেখানো</li>
        <li>একটি shopping cart-এর সব product বের করা</li>
        <li>database/API থেকে পাওয়া সব data process করা</li>
        <li>1, 2, 3, 4... এভাবে counting করা</li>
        <li>কোনো condition পূরণ না হওয়া পর্যন্ত কাজ চালানো</li>
      </ul>
      <p class="text-p" style="margin-top: 4px;">প্রতিবার আলাদা করে code লিখলে:</p>
      <div class="code-box" style="margin: 4px 0 6px 0;">
        <div class="code-top"><span>JavaScript</span><span>Without Loop (Inefficient)</span></div>
        <pre><span class="syn-fn">console</span>.log(<span class="syn-num">1</span>);
<span class="syn-fn">console</span>.log(<span class="syn-num">2</span>);
<span class="syn-fn">console</span>.log(<span class="syn-num">3</span>);
<span class="syn-fn">console</span>.log(<span class="syn-num">4</span>);
<span class="syn-fn">console</span>.log(<span class="syn-num">5</span>);</pre>
      </div>
      <p class="text-p" style="margin-top: 4px; font-weight: 600; color: #0369a1;">এভাবে 100 বার লিখতে হবে। 😵 তার পরিবর্তে <strong>Loop</strong> ব্যবহার করি।</p>
    </div>
""")

# Render all 52 numbered sections
for idx, sec_text in enumerate(all_secs, start=1):
    ...
    m = re.match(r'^#\s+(5\.\d+)\s+(.*)$', header_line)
    if m:
        sec_num, sec_title = m.group(1), m.group(2)
    else:
        sec_num, sec_title = f"5.{idx}", header_line.lstrip('#').strip()

    # Determine card styling for mini-projects
    card_style = ""
    badge_style = ""
    if "Mini Project" in sec_title:
...
# Part 07: Cheat Sheet, Differences, Practice Set & Summary
html_parts.append('    <div class="part-banner">Part 07 — Comprehensive Cheat Sheet, Comparisons &amp; Practice Set</div>\n')

# 1. Cheat sheet card
cheat_body = parse_section_body(post_cheat_md.replace('# 🧠 Chapter 5 Cheat Sheet', '').strip())
html_parts.append(f"""    <!-- Chapter 5 Cheat Sheet -->
    <div class="study-card" style="border-left: 4px solid #f59e0b;">
      <div class="card-title"><span class="badge-num" style="background: #d97706;">CHEAT SHEET</span> 🧠 Chapter 5 Cheat Sheet</div>
{cheat_body}
    </div>
""")

# 2. Key Differences
diff_body = parse_section_body(post_diff_md.replace('# 🔥 সবচেয়ে গুরুত্বপূর্ণ পার্থক্য', '').strip())
html_parts.append(f"""    <!-- Key Differences -->
    <div class="study-card" style="border-left: 4px solid #ef4444;">
      <div class="card-title"><span class="badge-num" style="background: #dc2626;">COMPARISON</span> 🔥 সবচেয়ে গুরুত্বপূর্ণ পার্থক্য (Key Loop Differences)</div>
{diff_body}
    </div>
""")
...
html_parts.append("""    <!-- Practice Set -->
    <div class="study-card" style="border-left: 4px solid #0284c7;">
      <div class="card-title"><span class="badge-num">PRACTICE LAB</span> 📝 Chapter 5 — Practice Set</div>
      <h3 style="font-size: 13px; font-weight: 700; color: var(--navy-mid); margin: 10px 0 6px 0;">Basic Questions</h3>
""")

# Parse Basic Questions
parts_prac = re.split(r'\n(?=##\s+💻\s+Coding Practice)', post_practice_md)
basic_md = parts_prac[0]
coding_md = parts_prac[1] if len(parts_prac) > 1 else ""

basic_items = re.findall(r'\*\*(\d+)\.\*\*\s+(.*?)(?=\n\s*\*\*\d+\.|\Z)', basic_md, flags=re.S)
for b_num, b_text in basic_items:
    clean_q = b_text.strip().replace('---', '').strip()
    html_parts.append(f"""      <div class="practice-item">
        <strong>{b_num}.</strong> {inline_format(clean_q)}
      </div>""")

html_parts.append("""      <h3 style="font-size: 13px; font-weight: 700; color: var(--navy-mid); margin: 16px 0 8px 0;">💻 Coding Practice</h3>
""")

coding_probs = re.split(r'\n(?=###\s+Problem\s+\d+)', coding_md)
for c_prob in coding_probs:
    if not c_prob.strip():
        continue
    c_lines = c_prob.strip().split('\n')
    prob_title = c_lines[0].replace('###', '').strip()
    prob_body = '\n'.join(c_lines[1:])
    rendered_prob = parse_section_body(prob_body)
    html_parts.append(f"""      <div class="practice-item" style="background: white; border: 1px solid var(--border-card); border-left: 4px solid var(--blue-accent); margin-bottom: 10px;">
        <h4 style="font-size: 12px; font-weight: 700; color: var(--navy-mid); margin-bottom: 4px;">{inline_format(prob_title)}</h4>
{rendered_prob}
      </div>""")
...
summary_lines = post_summary_md.replace('# 🎯 Chapter 5-এর মূল শিক্ষা', '').strip().split('\n')
# Split summary vs next preview
summary_text = '\n'.join(summary_lines)
parts_summary = re.split(r'\n(?=পরের chapter-এ আমরা যাব)', summary_text)
core_learnings_md = parts_summary[0]
next_preview_md = parts_summary[1] if len(parts_summary) > 1 else ""

rendered_summary = parse_section_body(core_learnings_md)

html_parts.append(f"""    <!-- Chapter 5 Core Learnings -->
    <div class="study-card" style="border-left: 4px solid #8b5cf6; background: #faf5ff;">
      <div class="card-title"><span class="badge-num" style="background: #7c3aed;">SUMMARY</span> 🎯 Chapter 5-এর মূল শিক্ষা</div>
{rendered_summary}
    </div>
""")
...
if next_preview_md.strip():
    ...
    <div class="study-card" style="border-left: 4px solid #0284c7; background: #f0f9ff;">
      <div class="card-title"><span class="badge-num">NEXT CHAPTER</span> 🚀 Chapter 6 Preview — Functions</div>
...
# Write to file
target_file = r'Code/Chapter-05-Loops-Iteration.html'
