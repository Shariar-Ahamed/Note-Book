...
def parse_section_body(body_md, sec_num=""):
    ...
    for b in blocks:
        ...
        if b_type == 'subheading':
            ...
            elif 'visual' in sub.lower():
                out_html.append('<span class="tag-label purple">Visual Diagram</span>')
            ...
            if lang == 'javascript' or (not lang and ('let ' in code_str or 'var ' in code_str or 'const ' in code_str or 'console.log' in code_str or 'function ' in code_str)):
                top_label = "JavaScript"
                sub_title = "Code"
                if last_subheading and last_subheading.lower() not in ('output', 'output:', 'code'):
  ...
  <pre>{highlight_js(code_str)}</pre>
                ...
                is_output = ('output' in last_subheading.lower()) or (code_str.strip() in ('Shariar', 'Ripon', 'ReferenceError', 'undefined', '10', '20', '100', 'Accessible', 'Hello JavaScript', 'TypeError'))
                has_diagram_chars = any(c in code_str for c in ('↓', '→', '┌', '└', '├', '│', '┴', '┬', '├──', '└──', '▲', '▼', '|', '↑')) or ('TDZ' in code_str) or ('Company' in code_str)
  ...
  <pre>{html.escape(code_str)}</pre>
            ...
            out_html.append(f'<div class="def-box english" style="margin: 6px 0;"><strong>Core Rule:</strong> {inline_format(b[1])}</div>')
            ...
            p_text = b[1]
            if p_text.startswith('**Scope** মানে হলো') or p_text.startswith('**Hoisting** হলো') or p_text.startswith('**Temporal Dead Zone** হলো') or p_text.startswith('**Lexical Scope** হলো') or p_text.startswith('**Shadowing** হলো'):
                out_html.append(f'<div class="def-box bangla">{inline_format(p_text)}</div>')
            elif p_text.startswith('**মনে রেখো:**') or p_text.startswith('**Note:**') or p_text.startswith('**Rule:**') or 'Golden Rule' in p_text:
                out_html.append(f'<div class="memory-box">{inline_format(p_text)}</div>')
            elif p_text.startswith('**ভুল:**') or p_text.startswith('**সতর্কতা:**') or p_text.startswith('**Warning:**') or p_text.startswith('⚠️'):
...
splits = re.split(r'\n(?=#\s+7\.\d+\s+)', raw_md)
opening_md = splits[0]
numbered_secs = splits[1:47]

sec47_and_after = splits[47]
post_parts = re.split(r'\n(?=#{1,2}\s+[🧠🔥📝🎯📘])', sec47_and_after)
sec47_md = post_parts[0]
post_cheat_md = post_parts[1]
post_must_md = post_parts[2]
post_practice_md = post_parts[3]
post_summary_md = post_parts[4]
post_next_md = post_parts[5] if len(post_parts) > 5 else ""

all_secs = numbered_secs + [sec47_md]

# Part definitions: (trigger_sec_num, banner_text)
PART_TRIGGERS = {
    1: "Part 01 — Core Scopes: Global, Function & Block Scopes (7.1 – 7.10)",
    11: "Part 02 — Scope Chain, Lexical Scope & Variable Shadowing (7.11 – 7.19)",
    20: "Part 03 — Hoisting Mechanics & Temporal Dead Zone (TDZ) (7.20 – 7.30)",
    31: "Part 04 — Redeclaration, Reassignment & Mutability (7.31 – 7.38)",
    39: "Part 05 — Scope & Hoisting Combined, Golden Rules & Best Practices (7.39 – 7.47)",
}
...
<head>
  ...
  <title>JS Master Study Documentation — Chapter 07: Scope &amp; Hoisting</title>
...
<body>
  ...
  <div class="doc-page">
    ...
    <header class="master-banner">
      ...
      <div class="banner-sub">CHAPTER 07: SCOPE &amp; HOISTING</div>
      <div class="meta-grid">
        <div class="meta-item">
          <div class="meta-label">Total Sections</div>
          <div class="meta-val">47 Modules + Practice</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Curriculum Part</div>
          <div class="meta-val">Part 03 — Core Mechanics (3/3)</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Core Mechanics</div>
          <div class="meta-val">Lexical Scope, Closures &amp; Hoisting</div>
        </div>
        <div class="meta-item">
    ...
    <div class="study-card" style="border-left: 4px solid var(--blue-accent); background: #f8fafc; padding: 10px 14px; margin-bottom: 8px;">
      <h2 style="font-size: 13px; color: var(--navy-mid); margin-bottom: 4px;">Scope &amp; Hoisting — Scope এবং Hoisting</h2>
      <p class="text-p">Chapter 6-এ আমরা <strong>Functions</strong> শিখেছি। এখন JavaScript-এর খুব গুরুত্বপূর্ণ একটি বিষয় হলো <strong>Scope</strong> এবং <strong>Hoisting</strong>।</p>
      <p class="text-p" style="font-weight: 600; color: #0369a1;">এই chapter ভালোভাবে বুঝলে পরবর্তীতে <strong>Closures, Modules, Async JavaScript, React, Node.js</strong>—সবকিছু বুঝতে অনেক সহজ হবে।</p>
    </div>
""")

# Render all 47 numbered sections
for idx, sec_text in enumerate(all_secs, start=1):
    ...
    m = re.match(r'^#\s+(7\.\d+)\s+(.*)$', header_line)
    if m:
        sec_num, sec_title = m.group(1), m.group(2)
    else:
        sec_num, sec_title = f"7.{idx}", header_line.lstrip('#').strip()

    card_style = ""
    badge_style = ""
    if "Best Practice" in sec_title or "Golden Rule" in sec_title:
...
# Part 06: Cheat Sheet, Must Know, Practice Set & Summary
html_parts.append('    <div class="part-banner">Part 06 — Comprehensive Cheat Sheet, Must Know &amp; Practice Lab</div>\n')

# 1. Quick Cheat Sheet
cheat_body = parse_section_body(post_cheat_md.replace('# 🧠 Quick Cheat Sheet', '').strip())
html_parts.append(f"""    <!-- Chapter 7 Quick Cheat Sheet -->
    <div class="study-card" style="border-left: 4px solid #f59e0b;">
      <div class="card-title"><span class="badge-num" style="background: #d97706;">CHEAT SHEET</span> 🧠 Quick Cheat Sheet</div>
{cheat_body}
    </div>
""")

# 2. Must Know
must_body = parse_section_body(post_must_md.replace('# 🔥 Chapter 7 — Must Know', '').strip())
html_parts.append(f"""    <!-- Chapter 7 Must Know -->
    <div class="study-card" style="border-left: 4px solid #ef4444;">
      <div class="card-title"><span class="badge-num" style="background: #dc2626;">MUST KNOW</span> 🔥 Chapter 7 — Must Know</div>
{must_body}
    </div>
""")
...
html_parts.append("""    <!-- Practice Set -->
    <div class="study-card" style="border-left: 4px solid #0284c7;">
      <div class="card-title"><span class="badge-num">PRACTICE LAB</span> 📝 Practice Set</div>
""")

# Parse Practice Problems
probs = re.split(r'\n(?=###\s+Practice\s+\d+)', post_practice_md.replace('# 📝 Practice Set', ''))
for p in probs:
    if not p.strip():
        continue
...
summary_clean = post_summary_md.replace('## 🎯 Chapter 7-এর মূল ধারণা এক লাইনে', '').strip()
rendered_summary = parse_section_body(summary_clean)

html_parts.append(f"""    <!-- Chapter 7 Final Summary -->
    <div class="study-card" style="border-left: 4px solid #8b5cf6; background: #faf5ff;">
      <div class="card-title"><span class="badge-num" style="background: #7c3aed;">SUMMARY</span> 🎯 Chapter 7-এর মূল ধারণা এক লাইনে</div>
{rendered_summary}
    </div>
""")

if post_next_md.strip():
    next_clean = post_next_md.replace('# 📘 Chapter 8 — Strings', '').strip()
    html_parts.append(f"""    <!-- Next Chapter Preview -->
    <div class="study-card" style="border-left: 4px solid #0284c7; background: #f0f9ff;">
      <div class="card-title"><span class="badge-num">NEXT CHAPTER</span> 🚀 Chapter 8 Preview — Strings</div>
      <p class="text-p" style="font-size: 12px; line-height: 1.6; color: #0c4a6e;">
        {inline_format(next_clean)}
      </p>
    </div>
""")
...
# Write HTML file
target_file = r'Code/Chapter-07-Scope-Hoisting.html'
