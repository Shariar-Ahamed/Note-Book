...
with open(r'temp/ch-14.md', 'r', encoding='utf-8') as f:
...
def highlight_js(code_str):
    token_spec = [
        ...
        ('BUILTIN', r'\b(?:console|Error|SyntaxError|ReferenceError|TypeError|RangeError|URIError|EvalError|AggregateError|Math|Array|Object|String|Number|BigInt|Intl|RegExp|Boolean|Function|Symbol|Map|Set|WeakMap|WeakSet|parseInt|parseFloat|prompt|alert|JSON)\b'),
        ('NUMBER', r'\b\d+(?:\.\d+)?\b'),
        ('OTHER', r'[^\s\w]+|\w+|\s+'),
    ]
...
def parse_section_body(body_md, sec_num=""):
  ...
  <div class="def-text">{inline_format(b[1])}</div>
            ...
            if "Output:" in txt or txt.strip() == "Output:":
                continue
            if txt.startswith("⚠️") or "Warning" in txt or "সতর্কতা" in txt or "Error" in txt or "Bad" in txt or "সমস্যা" in txt:
            ...
            is_ascii = any(c in raw_code for c in ['├──', '└──', '│', '──', '┌', '└', '↓', '→', '▼', '▲', '┼', '---------------------']) or (lang == 'text' and any(k in raw_code.lower() for k in ["error", "console", "devtools", "try", "catch", "problem", "bug", "account", "pin"]))
  ...
  <pre>{html.escape(raw_code)}</pre>
                ...
                last_subheading = ""
            elif lang in ('javascript', 'js') or (not lang and any(k in raw_code for k in ['const ', 'let ', 'var ', 'function', 'console.', 'try ', 'catch', 'finally', 'throw ', 'class ', 'new Error'])):
...
splits = re.split(r'\n(?=#\s+14\.\d+\s+)', raw_md)
opening_md = splits[0]
numbered_secs = splits[1:54] # 14.1 to 14.53

sec54_and_after = splits[54] # 14.54 and post sections
post_splits = re.split(r'\n(?=#\s+[🔥🧠📝🎯])', sec54_and_after)
sec54_md = post_splits[0]
must_know_md = post_splits[1]
quick_cheat_md = post_splits[2]
practice_set_md = post_splits[3]
final_summary_md = post_splits[4]

all_secs = numbered_secs + [sec54_md]

# Part definitions: (trigger_sec_num, banner_text)
PART_TRIGGERS = {
    1: "Part 01 — JavaScript Error Architecture &amp; Error Types (14.1 – 14.10)",
    11: "Part 02 — Exception Handling: try, catch, finally &amp; throw (14.11 – 14.26)",
    27: "Part 03 — Modern Debugging &amp; Console API (14.27 – 14.37)",
    38: "Part 04 — Browser DevTools, Breakpoints &amp; Call Stack (14.38 – 14.43)",
    44: "Part 05 — Advanced Error Handling, Best Practices &amp; Mini Project (14.44 – 14.54)",
}
...
<head>
  ...
  <title>JS Master Study Documentation — Chapter 14: Error Handling &amp; Debugging</title>
...
<body>
  ...
  <div class="doc-page">
    ...
    <header class="master-banner">
      ...
      <div class="banner-sub">CHAPTER 14: ERROR HANDLING &amp; DEBUGGING — ARCHITECTURE, PATTERNS &amp; TOOLS</div>
      <div class="meta-grid">
        <div class="meta-item">
          <div class="meta-label">Total Sections</div>
          <div class="meta-val">54 Modules + Practice</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Curriculum Part</div>
          <div class="meta-val">Part 08 — Quality, Reliability &amp; Debugging</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Core Mechanics</div>
          <div class="meta-val">try/catch, Custom Errors, DevTools &amp; Console</div>
        </div>
        <div class="meta-item">
    ...
    <div class="study-card" style="border-left: 4px solid var(--blue-accent); background: #f8fafc; padding: 10px 14px; margin-bottom: 8px;">
      <h2 style="font-size: 13px; color: var(--navy-mid); margin-bottom: 4px;">Error Handling &amp; Debugging — Error মোকাবিলা ও Debugging</h2>
      <p class="text-p">এখন আমরা JavaScript-এর খুব গুরুত্বপূর্ণ একটি অংশ শিখব: <strong>Error Handling এবং Debugging</strong>। বাস্তব জীবনের application-এ user ভুল input দিতে পারে, API fail করতে পারে, কোনো variable ভুল হতে পারে, server response না-ও আসতে পারে। তখন program যেন পুরোপুরি crash না করে এবং developer যেন সমস্যাটা সহজে খুঁজে বের করতে পারে—এই কাজের জন্য Error Handling ও Debugging ব্যবহার করা হয়।</p>
      <div class="def-box" style="margin: 4px 0;">
        <div class="def-text">Error Detection &nbsp;→&nbsp; Graceful Handling (try...catch) &nbsp;→&nbsp; Clean Recovery (finally) &nbsp;→&nbsp; Root-Cause Debugging</div>
      </div>
      <p class="text-p" style="font-weight: 600; color: #0369a1; margin-top: 4px;">নির্ভুল ও ক্র্যাশ-মুক্ত প্রফেশনাল ওয়েব অ্যাপ্লিকেশন তৈরির জন্য এই অধ্যায়টি অপরিহার্য। 🔥</p>
    </div>
""")

# Render all 54 numbered sections
for idx, sec_text in enumerate(all_secs, start=1):
    ...
    m = re.match(r'^#\s+(14\.\d+)\s+(.*)$', header_line)
    if m:
        sec_num, sec_title = m.group(1), m.group(2)
    else:
        sec_num, sec_title = f"14.{idx}", header_line.lstrip('#').strip()

    card_style = ""
    badge_style = ""
    if "গুরুত্বপূর্ণ" in sec_title or "Real-Life" in sec_title or "Mini Project" in sec_title or "বনাম" in sec_title:
        card_style = ' style="border-left: 4px solid #0284c7; background: #ffffff;"'
        badge_style = ' style="background: #0284c7;"'

    # Strict Page 1 Balance: Section 14.1 Card must break cleanly before Section 14.2
    if idx == 1:
        if card_style:
...
html_parts.append('    <div class="part-banner">Part 06 — Must Know, Quick Cheat Sheet, Practice Lab &amp; Final Summary</div>\n')

# 1. Must Know
must_know_body = parse_section_body(re.sub(r'^#\s+🔥\s+14\.55\s+Must Know', '', must_know_md).strip())
html_parts.append(f"""    <!-- 14.55 Must Know -->
    <div class="study-card" style="border-left: 4px solid #ef4444; background: #ffffff;">
      <div class="card-title"><span class="badge-num" style="background: #dc2626;">MUST KNOW</span> 🔥 14.55 Must Know</div>
{must_know_body}
    </div>
""")
...
html_parts.append("""    <!-- Practice Set -->
    <div class="study-card" style="border-left: 4px solid #0284c7;">
      <div class="card-title"><span class="badge-num">PRACTICE LAB</span> 📝 Practice Set — Chapter 14</div>
""")
...
summary_text = re.sub(r'^#\s+🎯\s+Chapter 14-এর মূল বিষয়', '', final_summary_md).strip()
parts_p = re.split(r'\n(?=এখন JavaScript-এর পরবর্তী|Chapter 15)', summary_text)
...
html_parts.append(f"""    <!-- Chapter 14 Final Summary -->
    <div class="study-card" style="border-left: 4px solid #8b5cf6; background: #faf5ff;">
      <div class="card-title"><span class="badge-num" style="background: #7c3aed;">SUMMARY</span> 🎯 Chapter 14 Final Summary — Error Handling Architecture</div>
{rendered_summary}
    </div>
""")
...
if next_preview_md:
    ...
    <div class="study-card" style="border-left: 4px solid #0284c7; background: #f0f9ff;">
      <div class="card-title"><span class="badge-num">NEXT CHAPTER</span> 📘 Chapter 15 Preview — DOM (Document Object Model)</div>
...
# Write HTML file
target_file = r'Code/Chapter-14-Error-Handling-Debugging.html'
