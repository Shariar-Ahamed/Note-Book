...
def parse_section_body(body_md, sec_num=""):
    ...
    for b in blocks:
        ...
        if b_type == 'subheading':
            ...
            elif 'flow' in sub.lower():
                out_html.append('<span class="tag-label cyan">Execution Flow</span>')
            else:
            ...
            if lang == 'javascript' or (not lang and ('function ' in code_str or 'return ' in code_str or 'console.log' in code_str or '=>' in code_str)):
  ...
  <pre>{highlight_js(code_str)}</pre>
                ...
                is_output = ('output' in last_subheading.lower()) or (code_str.strip() in ('Even', 'Odd', '25', '30', '64', '120', '150', '73500', 'Hello Shariar'))
                has_diagram_chars = any(c in code_str for c in ('↓', '→', '┌', '└', '├', '│', '┴', '┬', '├──', '└──')) or ('<=' in code_str and 'true' in code_str) or ('=' in code_str and 'a =' in code_str)
  ...
  <pre>{html.escape(code_str)}</pre>
            ...
            out_html.append(f'<div class="def-box english" style="margin: 6px 0;"><strong>Core Concept:</strong> {inline_format(b[1])}</div>')
            ...
            p_text = b[1]
            if p_text.startswith('**Function** হলো') or p_text.startswith('**Parameter** হলো') or p_text.startswith('**Argument** হলো') or p_text.startswith('**Closure** হলো') or p_text.startswith('**Callback** হলো') or p_text.startswith('**Pure Function** হলো'):
...
splits = re.split(r'\n(?=#\s+6\.\d+\s+)', raw_md)
opening_md = splits[0]
numbered_secs = splits[1:66]

sec66_and_after = splits[66]
post_parts = re.split(r'\n(?=#\s+[🧠🔥📝💻🎯])', sec66_and_after)
sec66_md = post_parts[0]
post_cheat_md = post_parts[1]
post_must_md = post_parts[2]
post_practice_md = post_parts[3]
post_coding_md = post_parts[4]
post_summary_md = post_parts[5]

all_secs = numbered_secs + [sec66_md]

# Part definitions: (trigger_sec_num, banner_text)
PART_TRIGGERS = {
    1: "Part 01 — Core Function Mechanics, Parameters & Arguments (6.1 – 6.12)",
    13: "Part 02 — Return Mechanism, Call Stacks & Default Parameters (6.13 – 6.20)",
    21: "Part 03 — Function Expressions, Hoisting & Arrow Functions (6.21 – 6.30)",
    31: "Part 04 — Anonymous Functions, IIFE & First-Class Citizens (6.31 – 6.37)",
    38: "Part 05 — Callbacks, Higher-Order Functions & Rest Parameters (6.38 – 6.51)",
    52: "Part 06 — Scope, Recursion, Best Practices & Mini-Projects (6.52 – 6.66)",
}
...
<head>
  ...
  <title>JS Master Study Documentation — Chapter 06: Functions</title>
...
<body>
  ...
  <div class="doc-page">
    ...
    <header class="master-banner">
      ...
      <div class="banner-sub">CHAPTER 06: FUNCTIONS &amp; EXECUTION ENGINE</div>
      <div class="meta-grid">
        <div class="meta-item">
          <div class="meta-label">Total Sections</div>
          <div class="meta-val">66 Modules + Projects</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Curriculum Part</div>
          <div class="meta-val">Part 03 — Core Mechanics (2/3)</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Core Mechanics</div>
          <div class="meta-val">Scope, Execution Context &amp; Closures</div>
        </div>
        <div class="meta-item">
    ...
    <div class="study-card" style="border-left: 4px solid var(--blue-accent); background: #f8fafc; padding: 10px 14px; margin-bottom: 8px;">
      <h2 style="font-size: 13px; color: var(--navy-mid); margin-bottom: 4px;">Functions — Code একবার লিখে বারবার ব্যবহার করার উপায়</h2>
      <p class="text-p">JavaScript শেখার ক্ষেত্রে <strong>Function</strong> একটি অত্যন্ত গুরুত্বপূর্ণ concept। তুমি যদি JavaScript দিয়ে:</p>
      <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 4px 12px; margin: 4px 0 6px 12px; font-size: 11px;">
        <div>• Website</div>
        <div>• Web App</div>
      ...
      <p class="text-p" style="font-weight: 600; color: #0369a1;">ইত্যাদি বানাতে চাও, তাহলে Function ভালোভাবে বুঝতেই হবে।</p>
    </div>
""")
...
for idx, sec_text in enumerate(all_secs, start=1):
    ...
    m = re.match(r'^#\s+(6\.\d+)\s+(.*)$', header_line)
    if m:
        sec_num, sec_title = m.group(1), m.group(2)
    else:
        sec_num, sec_title = f"6.{idx}", header_line.lstrip('#').strip()
...
# Part 07: Cheat Sheet, Must Remember, Practice Set & Summary
html_parts.append('    <div class="part-banner">Part 07 — Comprehensive Cheat Sheet, Must Remember &amp; Practice Lab</div>\n')

# 1. Quick Cheat Sheet
cheat_body = parse_section_body(post_cheat_md.replace('# 🧠 Chapter 6 — Quick Cheat Sheet', '').strip())
html_parts.append(f"""    <!-- Chapter 6 Quick Cheat Sheet -->
    <div class="study-card" style="border-left: 4px solid #f59e0b;">
      <div class="card-title"><span class="badge-num" style="background: #d97706;">CHEAT SHEET</span> 🧠 Chapter 6 — Quick Cheat Sheet</div>
{cheat_body}
    </div>
""")

# 2. Must Remember
must_body = parse_section_body(post_must_md.replace('# 🔥 Chapter 6 — Must Remember', '').strip())
html_parts.append(f"""    <!-- Chapter 6 Must Remember -->
    <div class="study-card" style="border-left: 4px solid #ef4444;">
      <div class="card-title"><span class="badge-num" style="background: #dc2626;">MUST REMEMBER</span> 🔥 Chapter 6 — Must Remember (Core Patterns)</div>
{must_body}
    </div>
""")
...
html_parts.append("""    <!-- Practice Set -->
    <div class="study-card" style="border-left: 4px solid #0284c7;">
      <div class="card-title"><span class="badge-num">PRACTICE LAB</span> 📝 Chapter 6 — Practice Set</div>
      <h3 style="font-size: 12.5px; font-weight: 700; color: var(--navy-mid); margin: 8px 0 4px 0;">Basic Questions</h3>
""")

# Parse Basic Questions
basic_items = re.findall(r'\*\*(\d+)\.\*\*\s+(.*?)(?=\n\s*\*\*\d+\.|\Z)', post_practice_md, flags=re.S)
...
html_parts.append("""      <h3 style="font-size: 12.5px; font-weight: 700; color: var(--navy-mid); margin: 14px 0 6px 0;">💻 Coding Practice</h3>
""")

# Parse Coding Problems
coding_probs = re.split(r'\n(?=###\s+Problem\s+\d+)', post_coding_md)
...
# 4. Final Mental Map & Key Takeaways
rendered_summary = parse_section_body(post_summary_md.replace('# 🎯 Chapter 6 শেষ করার পর তোমার যা পরিষ্কার হওয়া উচিত', '').strip())

html_parts.append(f"""    <!-- Chapter 6 Final Mental Map -->
    <div class="study-card" style="border-left: 4px solid #8b5cf6; background: #faf5ff;">
      <div class="card-title"><span class="badge-num" style="background: #7c3aed;">SUMMARY</span> 🎯 Chapter 6 শেষ করার পর তোমার যা পরিষ্কার হওয়া উচিত</div>
{rendered_summary}
    </div>
""")
...
# Write HTML file
target_file = r'Code/Chapter-06-Functions.html'
