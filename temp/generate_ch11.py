...
def highlight_js(code_str):
    token_spec = [
        ...
        ('STRING_DBL', r'"(?:\\.|[^"\\])*"'),
        ('STRING_SGL', r"'(?:\\.|[^'\\])*'"),
        ('KEYWORD', r'\b(?:for|while|do|break|continue|if|else|let|const|var|function|return|switch|case|default|of|in|new|typeof|delete|this|super|class|extends|get|set|static|instanceof|try|catch|finally|throw|await|async|yield)\b'),
        ('BOOL_NULL', r'\b(?:true|false|null|undefined|NaN)\b'),
        ('BUILTIN', r'\b(?:console|Math|Array|Object|String|Number|Boolean|Function|Symbol|parseInt|parseFloat|prompt|alert)\b'),
        ('NUMBER', r'\b\d+(?:\.\d+)?\b'),
        ('OTHER', r'[^\s\w]+|\w+|\s+'),
    ]
...
def parse_section_body(body_md, sec_num=""):
  ...
  <div class="def-text">{inline_format(b[1])}</div>
            ...
            elif "মনে রাখবে" in txt or "গুরুত্বপূর্ণ" in txt or "আয়ত্ত করবে" in txt or "পার্থক্য" in txt:
            ...
            is_ascii = any(c in raw_code for c in ['├──', '└──', '│', '──', '┌', '└', '↓', '→', '▼', '▲', '┼']) or (lang == 'text' and ("prototype" in raw_code.lower() or "student" in raw_code.lower() or "person" in raw_code.lower() or "object" in raw_code.lower()))
...
splits = re.split(r'\n(?=#\s+11\.\d+\s+)', raw_md)
opening_md = splits[0]
numbered_secs = splits[1:54] # 11.1 to 11.53

sec54_and_after = splits[54] # 11.54 and post sections
...
PART_TRIGGERS = {
    1: "Part 01 — Prototypes, Chains & Property Lookups (11.1 – 11.14)",
    15: "Part 02 — Constructor Functions, 'new' Keyword & Prototype Memory (11.15 – 11.29)",
    30: "Part 03 — Modern ES6 Classes, 'extends', 'super' & Overriding (11.30 – 11.39)",
    40: "Part 04 — Meta-Programming: Descriptors, DefineProperty & Getters/Setters (11.40 – 11.54)",
}
...
<head>
  ...
  <title>JS Master Study Documentation — Chapter 11: Prototypes &amp; Prototype Chain</title>
...
<body>
  ...
  <div class="doc-page">
    ...
    <header class="master-banner">
      ...
      <div class="banner-sub">CHAPTER 11: ADVANCED OBJECTS, PROTOTYPES &amp; CHAIN</div>
      <div class="meta-grid">
        <div class="meta-item">
        ...
        <div class="meta-item">
          ...
          <div class="meta-val">Part 05 — Advanced Object Architecture</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Core Mechanics</div>
          <div class="meta-val">Prototype Chain, 'new', ES6 Classes &amp; Descriptors</div>
        </div>
        <div class="meta-item">
    ...
    <div class="study-card" style="border-left: 4px solid var(--blue-accent); background: #f8fafc; padding: 10px 14px; margin-bottom: 8px;">
      <h2 style="font-size: 13px; color: var(--navy-mid); margin-bottom: 4px;">Advanced Objects: Prototypes &amp; Prototype Chain</h2>
      <p class="text-p">Chapter 10-এ আমরা Object-এর basic থেকে advanced অনেক কিছু শিখেছি। এবার JavaScript-এর সবচেয়ে গুরুত্বপূর্ণ <strong>Object-related advanced concept</strong>-গুলোর একটি শিখব:</p>
      <div class="def-box" style="margin: 4px 0;">
        <div class="def-text">Prototype → Prototype Chain → Constructor Function → <code>new</code> → <code>Object.create()</code> → Inheritance</div>
      </div>
      <p class="text-p" style="font-weight: 600; color: #0369a1; margin-top: 4px;">এগুলো ভালোভাবে বুঝলে পরে <strong>Class, OOP, Inheritance, React/Node.js-এর অনেক concept</strong> বুঝতে সহজ হবে।</p>
    </div>
""")
...
for idx, sec_text in enumerate(all_secs, start=1):
    ...
    m = re.match(r'^#\s+(11\.\d+)\s+(.*)$', header_line)
    if m:
        sec_num, sec_title = m.group(1), m.group(2)
    else:
        sec_num, sec_title = f"11.{idx}", header_line.lstrip('#').strip()

    card_style = ""
    badge_style = ""
    if "গুরুত্বপূর্ণ" in sec_title or "Real-Life" in sec_title or "class" in sec_title.lower() or "prototype" in sec_title.lower():
...
html_parts.append('    <div class="part-banner">Part 05 — Must Know, Quick Cheat Sheet, Practice Lab &amp; Summary</div>\n')

# 1. Must Know
must_know_body = parse_section_body(re.sub(r'^#\s+🔥\s+Chapter 11\s+—\s+Must Know', '', must_know_md).strip())
html_parts.append(f"""    <!-- Must Know -->
    <div class="study-card" style="border-left: 4px solid #ef4444; background: #ffffff;">
      <div class="card-title"><span class="badge-num" style="background: #dc2626;">MUST KNOW</span> 🔥 Chapter 11 — Must Know</div>
{must_know_body}
    </div>
""")
...
html_parts.append("""    <!-- Practice Set -->
    <div class="study-card" style="border-left: 4px solid #0284c7;">
      <div class="card-title"><span class="badge-num">PRACTICE LAB</span> 📝 Practice Set — Chapter 11</div>
""")
...
summary_text = re.sub(r'^#\s+🎯\s+Chapter 11 Final Summary', '', final_summary_md).strip()
parts_p = re.split(r'\n(?=\*\*Chapter 12|Chapter 12)', summary_text)
...
html_parts.append(f"""    <!-- Chapter 11 Final Summary -->
    <div class="study-card" style="border-left: 4px solid #8b5cf6; background: #faf5ff;">
      <div class="card-title"><span class="badge-num" style="background: #7c3aed;">SUMMARY</span> 🎯 Chapter 11 Final Summary — Object Prototype Model</div>
{rendered_summary}
    </div>
""")
...
if next_preview_md:
    ...
    <div class="study-card" style="border-left: 4px solid #0284c7; background: #f0f9ff;">
      <div class="card-title"><span class="badge-num">NEXT CHAPTER</span> 🚀 Chapter 12 Preview — Advanced Collections</div>
...
# Write HTML file
target_file = r'Code/Chapter-11-Prototypes.html'
