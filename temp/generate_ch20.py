...
with open(r'temp/ch-20.md', 'r', encoding='utf-8') as f:
...
def highlight_js(code_str):
    token_spec = [
        ...
        ('DOM_BUILTIN', r'\b(?:console|window|document|Math|Object|Array|Date|JSON|Promise|Error)\b'),
        ('DOM_METHOD', r'\b(?:add|subtract|multiply|divide|log|setItem|getItem|removeItem|clear|push|map|filter)\b'),
        ('NUMBER', r'\b\d+(?:\.\d+)?\b'),
        ('OTHER', r'[^\s\w]+|\w+|\s+'),
    ]
    tok_regex = '|'.join(f'(?P<{name}>{pattern})' for name, pattern in token_spec)
...
def parse_section_body(body_md, sec_num=""):
    ...
    for b in blocks:
        ...
        if b_type == 'subheading':
            ...
            elif "Syntax" in last_subheading or ".js" in last_subheading or ".html" in last_subheading:
  ...
  <div class="def-text">{inline_format(b[1])}</div>
            ...
            if "Output:" in txt or txt.strip() == "Output:":
                continue
            if txt.startswith("⚠️") or "Warning" in txt or "সতর্কতা" in txt or "সমস্যা" in txt or "বিপদ" in txt or "Error" in txt or "Circular" in txt:
            ...
            is_ascii = any(c in raw_code for c in ['├──', '└──', '│', '──', '┌', '└', '↓', '→', '▼', '▲', '┼', '---------------------']) or (lang == 'text' and any(k in raw_code.lower() for k in ["project", "app.js", "math.js", "cart", "auth", "utils", "index.html"]))
  ...
  <pre>{hl_code}</pre>
            ...
            elif lang in ('javascript', 'js') or (not lang and any(k in raw_code for k in ['import ', 'export ', 'const ', 'let ', 'function', 'console.log'])) or any(lang == m for m in ['import', 'export', 'app', 'math', 'await', 'add', 'index', 'auth', 'this', 'utils', 'as', 'password', 'api', 'storage', 'require', 'module', 'config', 'user']):
                hl_code = highlight_js(raw_code)
                label = f"{last_subheading}" if (last_subheading and ".js" in last_subheading) else "JavaScript Module"
                code_title = f"{sec_num} {label}" if sec_num else label
                out_html.append(f"""<div class="code-box">
  <div class="code-top"><span>{code_title}</span><span>ESM</span></div>
  <pre>{hl_code}</pre>
</div>""")
            else:
                out_html.append(f"""<div class="code-box">
...
opening_md = splits[0]
numbered_secs = splits[1:52] # 20.1 to 20.51 (51 sections)
must_know_md = splits[52]    # 20.52
quick_cheat_md = splits[53]  # 20.53
practice_set_md = splits[54] # 20.54
final_mental_md = splits[55] # 🎯 20.55 Final Mental Map

# Part definitions: (trigger_sec_num, banner_text)
PART_TRIGGERS = {
    1: "Part 01 — Module Architecture &amp; The ES Modules Standard (20.1 – 20.8)",
    9: "Part 02 — Named Exports, Default Exports &amp; Imports (20.9 – 20.17)",
    18: "Part 03 — Namespaces, Side-Effects &amp; Module Scoping (20.18 – 20.29)",
    30: "Part 04 — Dynamic Imports, Re-Exporting &amp; Project Architecture (20.30 – 20.43)",
    44: "Part 05 — Advanced Module Mechanics: Node.js, CommonJS vs ESM, Live Bindings (20.44 – 20.51)",
}
...
<head>
  ...
  <title>JS Master Study Documentation — Chapter 20: ES Modules (ESM)</title>
...
<body>
  ...
  <div class="doc-page">
    ...
    <header class="master-banner">
      ...
      <div class="banner-sub">CHAPTER 20: ES MODULES (ESM) — MODULAR ARCHITECTURE, IMPORT/EXPORT &amp; PATTERNS</div>
      <div class="meta-grid">
        <div class="meta-item">
          <div class="meta-label">Total Sections</div>
          <div class="meta-val">55 Modules + Practice</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Curriculum Part</div>
          <div class="meta-val">Part 14 — Modular Architecture &amp; Organization</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Core Mechanics</div>
          <div class="meta-val">import, export, Dynamic Import &amp; Live Bindings</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Reference Standard</div>
          <div class="meta-val">ECMAScript 2026 / TC39 / Node.js ESM</div>
        </div>
      </div>
    </header>
    ...
    <div class="study-card" style="border-left: 4px solid var(--blue-accent); background: #f8fafc; padding: 10px 14px; margin-bottom: 8px;">
      <h2 style="font-size: 13px; color: var(--navy-mid); margin-bottom: 4px;">Modules — বড় JavaScript Project-কে ছোট, Organized ও Reusable File-এ ভাগ করা</h2>
      <p class="text-p">প্রফেশনাল সফটওয়্যার ইঞ্জিনিয়ারিংয়ে কোনো বড় কোডবেস একটিমাত্র ফাইলে রাখা অসম্ভব। কোডবেসকে পরিষ্কার, রিইউজেবল, সহজে টেস্টেবল এবং মেইনটেইনেবল রাখতে জাভাস্ক্রিপ্টের অফিসিয়াল মডিউল স্ট্যান্ডার্ড <strong>ES Modules (ESM)</strong> ব্যবহার করা হয়।</p>
      <div class="def-box" style="margin: 4px 0;">
        <div class="def-text">Single Responsibility File &nbsp;→&nbsp; export (Named / Default) &nbsp;→&nbsp; Module Dependency Graph &nbsp;→&nbsp; import / Dynamic import()</div>
      </div>
      <p class="text-p" style="font-weight: 600; color: #0369a1; margin-top: 4px;">React, Vue, Next.js বা Node.js যেকোনো আধুনিক ফ্রন্টএন্ড ফ্রেমওয়ার্কের ভিত্তিই হলো এই মডিউল সিস্টেম। 🔥</p>
    </div>
""")

# Render all 51 numbered sections (20.1 to 20.51)
for idx, sec_text in enumerate(numbered_secs, start=1):
    ...
    m = re.search(r'\b(20\.\d+)\b\s*(.*)$', header_line)
    if m:
        sec_num, sec_title = m.group(1), m.group(2)
        sec_title = sec_title.strip('`* ')
    else:
        sec_num, sec_title = f"20.{idx}", header_line.lstrip('#').strip()

    card_style = ""
    badge_style = ""
    if "⚠️" in header_line or "Circular" in sec_title or "Error" in sec_title:
        card_style = ' style="border-left: 4px solid #ef4444; background: #ffffff;"'
        badge_style = ' style="background: #dc2626;"'
    elif "Project" in sec_title or "বনাম" in sec_title or "vs" in sec_title or "Structure" in sec_title or "Live Binding" in sec_title:
        card_style = ' style="border-left: 4px solid #0284c7; background: #ffffff;"'
        badge_style = ' style="background: #0284c7;"'

    # Strict Page 1 Balance: Section 20.1 Card must break cleanly before Section 20.2
    if idx == 1:
        if card_style:
...
html_parts.append('    <div class="part-banner">Part 06 — Module Mastery: Must Know, Cheat Sheet, Practice Lab &amp; Mental Map</div>\n')

# 1. Must Know (20.52)
must_know_body = parse_section_body(re.sub(r'^#\s+🔥\s+20\.52\s+Must Know', '', must_know_md).strip())
html_parts.append(f"""    <!-- 20.52 Must Know -->
    <div class="study-card" style="border-left: 4px solid #ef4444; background: #ffffff;">
      <div class="card-title"><span class="badge-num" style="background: #dc2626;">MUST KNOW</span> 🔥 20.52 Must Know — ES Modules Architecture</div>
{must_know_body}
    </div>
""")

# 2. Quick Cheat Sheet (20.53)
cheat_body = parse_section_body(re.sub(r'^#\s+🧠\s+20\.53\s+Quick Cheat Sheet', '', quick_cheat_md).strip())
html_parts.append(f"""    <!-- 20.53 Quick Cheat Sheet -->
    <div class="study-card" style="border-left: 4px solid #10b981; background: #f0fdf4;">
      <div class="card-title"><span class="badge-num" style="background: #059669;">CHEAT SHEET</span> 🧠 20.53 Quick Cheat Sheet — Import &amp; Export Syntax</div>
{cheat_body}
    </div>
""")

# 3. Practice Set (20.54)
html_parts.append("""    <!-- 20.54 Practice Set -->
    <div class="study-card" style="border-left: 4px solid #0284c7;">
      <div class="card-title"><span class="badge-num">PRACTICE LAB</span> 📝 20.54 Practice Set — 5 Modular Architecture Projects</div>
""")

prob_blocks = re.split(r'\n(?=###\s+Practice\s+\d+)', practice_set_md.replace('# 📝 20.54 Practice Set', ''))
...
# 4. Final Mental Map & Summary (20.55)
summary_text = re.sub(r'^#\s+🎯\s+20\.55\s+Final Mental Map', '', final_mental_md).strip()
rendered_summary = parse_section_body(summary_text)

html_parts.append(f"""    <!-- 20.55 Final Mental Map -->
    <div class="study-card" style="border-left: 4px solid #8b5cf6; background: #faf5ff;">
      <div class="card-title"><span class="badge-num" style="background: #7c3aed;">SUMMARY</span> 🎯 20.55 Final Mental Map — Module Organization &amp; Lifecycle</div>
{rendered_summary}
    </div>
""")
...
html_parts.append("""    <!-- Next Chapter Preview -->
    <div class="study-card" style="border-left: 4px solid #0284c7; background: #f0f9ff;">
      <div class="card-title"><span class="badge-num">NEXT CHAPTER</span> 📘 Chapter 21 Preview — Asynchronous JavaScript</div>
      <p class="text-p" style="font-size: 12px; line-height: 1.6; color: #0c4a6e;">
        পরবর্তী অধ্যায়ে আমরা জাভাস্ক্রিপ্টের অন্যতম কোর পিলার <strong>Asynchronous JavaScript</strong> নিয়ে বিস্তারিত শিখব—Event Loop, Call Stack, Callback Hell, Promises (<code>then</code>, <code>catch</code>, <code>finally</code>), Promise Combinators (<code>all</code>, <code>race</code>, <code>allSettled</code>, <code>any</code>), এবং আধুনিক <code>async/await</code> সিনট্যাক্স।
      </p>
    </div>
""")
...
# Write HTML file
target_file = r'Code/Chapter-20-ES-Modules.html'
