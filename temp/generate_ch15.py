...
with open(r'temp/ch-15.md', 'r', encoding='utf-8') as f:
...
def highlight_js(code_str):
    token_spec = [
        ...
        ('DOM_BUILTIN', r'\b(?:document|window|console|body|head|documentElement|Element|Node|NodeList|HTMLCollection|Event|CustomEvent|localStorage|sessionStorage|setTimeout|setInterval|clearTimeout|clearInterval|JSON|Math|Array|Object|String|Number)\b'),
        ('DOM_METHOD', r'\b(?:getElementById|getElementsByClassName|getElementsByTagName|querySelector|querySelectorAll|createElement|append|appendChild|prepend|before|after|remove|removeChild|replaceWith|cloneNode|getAttribute|setAttribute|hasAttribute|removeAttribute|classList|add|remove|toggle|contains|replace|closest|matches|addEventListener|removeEventListener|textContent|innerHTML|innerText|style|parentElement|children|childNodes|firstElementChild|lastElementChild|nextElementSibling|previousElementSibling|dataset)\b'),
        ('NUMBER', r'\b\d+(?:\.\d+)?\b'),
        ('OTHER', r'[^\s\w]+|\w+|\s+'),
    ]
    tok_regex = '|'.join(f'(?P<{name}>{pattern})' for name, pattern in token_spec)
...
def highlight_css(code_str):
    ...
    for line in code_str.split('\n'):
        if '{' in line and not line.strip().startswith('/*'):
            ...
            sel = parts[0]
            rest = parts[1]
            lines.append(f'<span class="syn-kw">{html.escape(sel)}</span>{{<span class="syn-str">{html.escape(rest)}</span>')
        elif ':' in line and not line.strip().startswith('/*'):
            parts = line.split(':', 1)
            prop = parts[0]
            val = parts[1]
            lines.append(f'  <span class="syn-fn">{html.escape(prop)}</span>:<span class="syn-str">{html.escape(val)}</span>')
        else:
...
def parse_section_body(body_md, sec_num=""):
    ...
    for b in blocks:
        ...
        if b_type == 'subheading':
            ...
            elif "মনে রাখো" in last_subheading or "মনে রাখবে" in last_subheading or "গুরুত্বপূর্ণ" in last_subheading:
  ...
  <div class="def-text">{inline_format(b[1])}</div>
            ...
            is_ascii = any(c in raw_code for c in ['├──', '└──', '│', '──', '┌', '└', '↓', '→', '▼', '▲', '┼', '---------------------']) or (lang == 'text' and any(k in raw_code.lower() for k in ["document", "html", "university", "department", "input", "task", "product"]))
  ...
  <pre>{hl_code}</pre>
                ...
                hl_code = highlight_css(raw_code)
                code_title = f"{sec_num} CSS" if sec_num else "CSS"
                out_html.append(f"""<div class="code-box">
  <div class="code-top"><span style="color:#38bdf8;">{code_title}</span><span>Stylesheet</span></div>
  <pre>{hl_code}</pre>
</div>""")
            elif lang in ('javascript', 'js') or (not lang and any(k in raw_code for k in ['document.', 'console.', 'let ', 'const ', 'function', 'addEventListener', 'element.', 'title.'])) or any(lang == m for m in ['classlist', 'queryselector', 'queryselectorall', 'innerhtml', 'textcontent', 'append', 'appendchild', 'children', 'closest', 'data', 'getelementbyid', 'getelementsbyclassname', 'getelementsbytagname', 'innertext', 'getattribute', 'setattribute', 'hasattribute', 'removeattribute', 'style', 'prepend', 'childnodes', 'matches', 'addeventlistener', 'dataset', 'firstelementchild', 'lastelementchild', 'nextelementsibling', 'previouselementsibling', 'parentelement', 'createelement', 'remove', 'replacewith', 'clonenode', 'domcontentloaded']):
...
# Split sections
splits = re.split(r'\n(?=#\s+(?:.*?\b15\.\d+\b|🧠\s+Quick|📝|🎯))', raw_md)

opening_md = splits[0]
numbered_secs = splits[1:79] # 15.1 to 15.78 (78 sections)
quick_cheat_md = splits[79]
practice_set_md = splits[80]
final_summary_md = splits[81]

# Part definitions: (trigger_sec_num, banner_text)
PART_TRIGGERS = {
    1: "Part 01 — DOM Architecture, Nodes &amp; the Document Object (15.1 – 15.10)",
    11: "Part 02 — DOM Element Selection &amp; Query Selectors (15.11 – 15.24)",
    25: "Part 03 — Content, Attributes &amp; Form Element Value Manipulation (15.25 – 15.37)",
    38: "Part 04 — CSS Styling &amp; ClassList Dynamic Manipulation (15.38 – 15.45)",
    46: "Part 05 — Element Creation, Insertion, Removal &amp; Cloning (15.46 – 15.57)",
    58: "Part 06 — DOM Traversal, Hierarchy &amp; Tree Navigation (15.58 – 15.67)",
    68: "Part 07 — Hands-On DOM Mini Projects &amp; Custom Dataset (15.68 – 15.71)",
    72: "Part 08 — DOM Engine Internals, Security &amp; Execution Lifecycle (15.72 – 15.78)",
}
...
<head>
  ...
  <title>JS Master Study Documentation — Chapter 15: DOM — Document Object Model</title>
...
<body>
  ...
  <div class="doc-page">
    ...
    <header class="master-banner">
      ...
      <div class="banner-sub">CHAPTER 15: DOM — DOCUMENT OBJECT MODEL — ARCHITECTURE, MANIPULATION &amp; INTERFACES</div>
      <div class="meta-grid">
        <div class="meta-item">
          <div class="meta-label">Total Sections</div>
          <div class="meta-val">78 Modules + Practice</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Curriculum Part</div>
          <div class="meta-val">Part 09 — Browser Environment &amp; DOM</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Core Mechanics</div>
          <div class="meta-val">Tree Traversal, Query Selection, Node Lifecycle &amp; classList</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Reference Standard</div>
          <div class="meta-val">WHATWG DOM Standard / ECMAScript 2026</div>
        </div>
      </div>
    </header>
    ...
    <div class="study-card" style="border-left: 4px solid var(--blue-accent); background: #f8fafc; padding: 10px 14px; margin-bottom: 8px;">
      <h2 style="font-size: 13px; color: var(--navy-mid); margin-bottom: 4px;">DOM — Document Object Model — HTML-এর সাথে JavaScript-এর যোগাযোগ</h2>
      <p class="text-p">এখন থেকে JavaScript-এর সবচেয়ে গুরুত্বপূর্ণ practical অংশগুলোর একটি শুরু হচ্ছে — <strong>DOM (Document Object Model)</strong>। এতদিন আমরা JavaScript-এর ভিতরের language concepts শিখেছি। এখন শিখব কীভাবে JavaScript দিয়ে <strong>HTML webpage-এর content, structure, style এবং elements পরিবর্তন করা যায়।</strong></p>
      <div class="def-box" style="margin: 4px 0;">
        <div class="def-text">HTML Document &nbsp;→&nbsp; Browser DOM Tree (Nodes &amp; Elements) &nbsp;→&nbsp; JavaScript API &nbsp;→&nbsp; Interactive Webpage</div>
      </div>
      <p class="text-p" style="font-weight: 600; color: #0369a1; margin-top: 4px;">ব্রাউজারে যেকোনো আধুনিক ডায়নামিক ওয়েব অ্যাপ্লিকেশন বা রিয়্যাক্ট ফ্রেমওয়ার্কের মূল ভিত্তি হলো এই DOM আর্কিটেকচার। 🔥</p>
    </div>
""")

# Render all 78 numbered sections
for idx, sec_text in enumerate(numbered_secs, start=1):
    ...
    # Match section number and title
    m = re.search(r'\b(15\.\d+)\b\s*(.*)$', header_line)
    if m:
        sec_num, sec_title = m.group(1), m.group(2)
        # remove prefix emojis from title if any
        sec_title = sec_title.strip('`* ')
    else:
        sec_num, sec_title = f"15.{idx}", header_line.lstrip('#').strip()
    ...
    if "⚠️" in header_line:
        ...
        card_style = ' style="border-left: 4px solid #f59e0b; background: #ffffff;"'
        badge_style = ' style="background: #d97706;"'
    elif "🧠" in header_line:
        card_style = ' style="border-left: 4px solid #8b5cf6; background: #faf5ff;"'
        badge_style = ' style="background: #7c3aed;"'
    elif "🔥" in header_line:
        card_style = ' style="border-left: 4px solid #ef4444; background: #ffffff;"'
        badge_style = ' style="background: #dc2626;"'
    elif "Mini Project" in sec_title or "বনাম" in sec_title or "Tree" in sec_title:
        card_style = ' style="border-left: 4px solid #0284c7; background: #ffffff;"'
        badge_style = ' style="background: #0284c7;"'

    # Strict Page 1 Balance: Section 15.1 Card must break cleanly before Section 15.2
    if idx == 1:
        if card_style:
...
# Part 09: Quick Cheat Sheet, Practice Lab & Final Summary
html_parts.append('    <div class="part-banner">Part 09 — Quick Cheat Sheet, Practice Lab (12 Problems) &amp; Final Summary</div>\n')
...
html_parts.append(f"""    <!-- Quick Cheat Sheet -->
    <div class="study-card" style="border-left: 4px solid #10b981; background: #f0fdf4;">
      <div class="card-title"><span class="badge-num" style="background: #059669;">CHEAT SHEET</span> 🧠 Quick Cheat Sheet — Core DOM Syntax</div>
{cheat_body}
    </div>
""")
...
html_parts.append("""    <!-- Practice Set -->
    <div class="study-card" style="border-left: 4px solid #0284c7;">
      <div class="card-title"><span class="badge-num">PRACTICE LAB</span> 📝 Chapter 15 Practice Set — 12 Hands-on Problems</div>
""")

# Parse Practice Problems: Beginner, Intermediate, Advanced
prob_blocks = re.split(r'\n(?=###\s+)', practice_set_md.replace('# 📝 Chapter 15 Practice Set', ''))
for pb in prob_blocks:
    ...
    # Split individual items like **1.**, **2.**, etc.
    q_splits = re.split(r'\n(?=\*\*\d+\.\*\*)', cat_body)
...
# 3. Final Summary & Next Chapter Preview
summary_text = re.sub(r'^#\s+🎯\s+Chapter 15 Final Summary', '', final_summary_md).strip()
parts_p = re.split(r'\n(?=\*\*পরবর্তী: Chapter 16)', summary_text)
...
html_parts.append(f"""    <!-- Chapter 15 Final Summary -->
    <div class="study-card" style="border-left: 4px solid #8b5cf6; background: #faf5ff;">
      <div class="card-title"><span class="badge-num" style="background: #7c3aed;">SUMMARY</span> 🎯 Chapter 15 Final Summary — DOM Architecture &amp; Lifecycle</div>
{rendered_summary}
    </div>
""")
...
if next_preview_md:
    ...
    <div class="study-card" style="border-left: 4px solid #0284c7; background: #f0f9ff;">
      <div class="card-title"><span class="badge-num">NEXT CHAPTER</span> 📘 Chapter 16 Preview — JavaScript Events &amp; Event Handling</div>
...
# Write HTML file
target_file = r'Code/Chapter-15-DOM.html'
