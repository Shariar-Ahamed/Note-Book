...
with open(r'temp/ch-13.md', 'r', encoding='utf-8') as f:
...
def highlight_js(code_str):
    token_spec = [
        ...
        ('BUILTIN', r'\b(?:console|Math|Array|Object|String|Number|BigInt|Intl|RegExp|Boolean|Function|Symbol|Map|Set|WeakMap|WeakSet|parseInt|parseFloat|prompt|alert|JSON)\b'),
        ('BIGINT', r'\b\d+n\b'),
        ('NUMBER', r'\b\d+(?:\.\d+)?\b'),
        ('OTHER', r'[^\s\w]+|\w+|\s+'),
    ]
...
def parse_section_body(body_md, sec_num=""):
  ...
  <div class="def-text">{inline_format(b[1])}</div>
            ...
            if "Output:" in txt or txt.strip() == "Output:":
                continue
            if txt.startswith("⚠️") or "Warning" in txt or "সতর্কতা" in txt or "Error" in txt or "TypeError" in txt or "Unsafe" in txt:
            ...
            is_ascii = any(c in raw_code for c in ['├──', '└──', '│', '──', '┌', '└', '↓', '→', '▼', '▲', '┼', '---------------------']) or (lang == 'text' and any(k in raw_code.lower() for k in ["date", "math", "number", "bigint", "intl", "regexp", "year", "month", "start:", "end:"]))
  ...
  <pre>{html.escape(raw_code)}</pre>
                ...
                last_subheading = ""
            elif lang in ('javascript', 'js') or (not lang and any(k in raw_code for k in ['const ', 'let ', 'var ', 'function', 'console.log', 'Math.', 'Number.', 'Date.', 'Intl.', 'new '])):
...
splits = re.split(r'\n(?=#\s+13\.\d+\s+)', raw_md)
opening_md = splits[0]
numbered_secs = splits[1:92] # 13.1 to 13.91

sec92_and_after = splits[92] # 13.92 and post sections
post_splits = re.split(r'\n(?=#\s+[🔥🧠📝🎯])', sec92_and_after)
sec92_md = post_splits[0]
must_know_md = post_splits[1]
quick_cheat_md = post_splits[2]
practice_set_md = post_splits[3]
final_mental_md = post_splits[4]

all_secs = numbered_secs + [sec92_md]

# Part definitions: (trigger_sec_num, banner_text)
PART_TRIGGERS = {
    1: "Part 01 — Date Object: Time, Calculations &amp; Intl.DateTimeFormat (13.1 – 13.20)",
    21: "Part 02 — Math Object: Arithmetic, Rounding &amp; Random Generators (13.21 – 13.39)",
    40: "Part 03 — Number &amp; BigInt: Safe Integers, Precision &amp; Large Numbers (13.40 – 13.56)",
    57: "Part 04 — Intl API: Internationalization, Currency &amp; Localization (13.57 – 13.64)",
    65: "Part 05 — RegExp &amp; String Utilities: Patterns, Validation &amp; Mini Project (13.65 – 13.92)",
}
...
<head>
  ...
  <title>JS Master Study Documentation — Chapter 13: Built-in Objects</title>
...
<body>
  ...
  <div class="doc-page">
    ...
    <header class="master-banner">
      ...
      <div class="banner-sub">CHAPTER 13: BUILT-IN OBJECTS — DATE, MATH, NUMBER, BIGINT, INTL &amp; REGEXP</div>
      <div class="meta-grid">
        <div class="meta-item">
          <div class="meta-label">Total Sections</div>
          <div class="meta-val">92 Modules + Practice</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Curriculum Part</div>
          <div class="meta-val">Part 07 — Core Built-in Standard Library</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Core Mechanics</div>
          <div class="meta-val">Date, Math, Number, BigInt, Intl &amp; RegExp</div>
        </div>
        <div class="meta-item">
    ...
    <div class="study-card" style="border-left: 4px solid var(--blue-accent); background: #f8fafc; padding: 10px 14px; margin-bottom: 8px;">
      <h2 style="font-size: 13px; color: var(--navy-mid); margin-bottom: 4px;">Built-in Objects: Date, Math, Number, BigInt, Intl &amp; RegExp</h2>
      <p class="text-p">Chapter 12-এ আমরা <strong>Map, Set, WeakMap, WeakSet</strong> শিখেছি। এবার JavaScript-এর built-in tools/objects শিখব। এগুলো practically প্রতিদিনের JavaScript development-এ কাজে লাগে:</p>
      <div class="def-box" style="margin: 4px 0;">
        <div class="def-text">Date &nbsp;•&nbsp; Math &nbsp;•&nbsp; Number &nbsp;•&nbsp; BigInt &nbsp;•&nbsp; Intl &nbsp;•&nbsp; RegExp &nbsp;•&nbsp; String Utilities</div>
      </div>
      <p class="text-p" style="font-weight: 600; color: #0369a1; margin-top: 4px;">তারিখ, গাণিতিক হিসাব, বড় সংখ্যা ও আন্তর্জাতিক ফরম্যাটিংয়ের ভিত্তি মজবুত করতে এই অধ্যায়টি অত্যন্ত গুরুত্বপূর্ণ। 🔥</p>
    </div>
""")

# Render all 92 numbered sections
for idx, sec_text in enumerate(all_secs, start=1):
    ...
    m = re.match(r'^#\s+(13\.\d+)\s+(.*)$', header_line)
    if m:
        sec_num, sec_title = m.group(1), m.group(2)
    else:
        sec_num, sec_title = f"13.{idx}", header_line.lstrip('#').strip()
    ...
    # Strict Page 1 Balance: Section 13.1 Card must break cleanly before Section 13.2
    if idx == 1:
        if card_style:
...
html_parts.append('    <div class="part-banner">Part 06 — Must Know, Quick Cheat Sheet, Practice Lab &amp; Mental Map</div>\n')

# 1. Must Know
must_know_body = parse_section_body(re.sub(r'^#\s+🔥\s+Chapter 13\s+—\s+Must Know', '', must_know_md).strip())
html_parts.append(f"""    <!-- Must Know -->
    <div class="study-card" style="border-left: 4px solid #ef4444; background: #ffffff;">
      <div class="card-title"><span class="badge-num" style="background: #dc2626;">MUST KNOW</span> 🔥 Chapter 13 — Must Know</div>
{must_know_body}
    </div>
""")
...
html_parts.append("""    <!-- Practice Set -->
    <div class="study-card" style="border-left: 4px solid #0284c7;">
      <div class="card-title"><span class="badge-num">PRACTICE LAB</span> 📝 Practice Set — Chapter 13</div>
""")
...
final_text = re.sub(r'^#\s+🎯\s+Chapter 13 Final Mental Map', '', final_mental_md).strip()
rendered_final = parse_section_body(final_text)
html_parts.append(f"""    <!-- Chapter 13 Final Mental Map -->
    <div class="study-card" style="border-left: 4px solid #8b5cf6; background: #faf5ff;">
      <div class="card-title"><span class="badge-num" style="background: #7c3aed;">MENTAL MAP</span> 🎯 Chapter 13 Final Mental Map — Architecture &amp; Core Patterns</div>
{rendered_final}
    </div>
""")
...
# Write HTML file
target_file = r'Code/Chapter-13-Built-in-Objects.html'
