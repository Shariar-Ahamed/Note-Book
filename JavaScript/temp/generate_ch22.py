...
with open(r'temp/ch-22.md', 'r', encoding='utf-8') as f:
...
def highlight_js(code_str):
    token_spec = [
        ...
        ('DOM_BUILTIN', r'\b(?:console|window|document|Math|Object|Array|Date|JSON|Promise|Error|Response|Request|Headers|URLSearchParams|URL|AbortController|FormData|Blob)\b'),
        ('DOM_METHOD', r'\b(?:setTimeout|clearTimeout|setInterval|clearInterval|fetch|abort|signal|append|get|getAll|set|has|delete|keys|values|entries|toString|json|text|blob|stringify|parse|log|error|warn|info|push|map|filter|reduce|addEventListener|status|ok)\b'),
        ('NUMBER', r'\b\d+(?:\.\d+)?\b'),
        ('OTHER', r'[^\s\w]+|\w+|\s+'),
    ]
...
def highlight_http(code_str):
    lines = code_str.splitlines()
    out_lines = []
    for line in lines:
        esc = html.escape(line)
        # Check HTTP method line
        m = re.match(r'^(GET|POST|PUT|PATCH|DELETE|OPTIONS|HEAD)\s+(.*)$', line)
        if m:
            out_lines.append(f'<span class="syn-kw">{m.group(1)}</span> <span class="syn-str">{html.escape(m.group(2))}</span>')
            continue
        # Status line: HTTP/1.1 200 OK
        m_stat = re.match(r'^(HTTP/[\d\.]+)\s+(\d+)\s+(.*)$', line)
        if m_stat:
            out_lines.append(f'<span class="syn-fn">{m_stat.group(1)}</span> <span class="syn-num">{m_stat.group(2)}</span> <span class="syn-bool">{html.escape(m_stat.group(3))}</span>')
            continue
        # Header line: Header-Name: value
        if ':' in line:
            parts = line.split(':', 1)
            out_lines.append(f'<span class="syn-fn">{html.escape(parts[0])}</span>:<span class="syn-str">{html.escape(parts[1])}</span>')
            continue
...
def parse_section_body(body_md, sec_num=""):
    ...
    for b in blocks:
        ...
        if b_type == 'subheading':
            ...
            elif "Syntax" in last_subheading or "Parameter" in last_subheading or "State" in last_subheading or "Header" in last_subheading:
  ...
  <div class="def-text">{inline_format(b[1])}</div>
            ...
            if "Output:" in txt or txt.strip() == "Output:":
                continue
            if txt.startswith("⚠️") or "Warning" in txt or "সতর্কতা" in txt or "সমস্যা" in txt or "বিপদ" in txt or "Error" in txt or "CORS" in txt:
            ...
            is_ascii = any(c in raw_code for c in ['├──', '└──', '│', '──', '┌', '└', '↓', '→', '▼', '▲', '┼', '---------------------']) or (lang == 'text' and any(k in raw_code.lower() for k in ["you", "frontend", "backend", "web api", "kitchen", "client"]))
  ...
  <pre>{html.escape(raw_code)}</pre>
                ...
                code_title = f"{sec_num} JSON Payload" if sec_num else "JSON Payload"
                out_html.append(f"""<div class="code-box">
  <div class="code-top"><span style="color:#fbbf24;">{code_title}</span><span>application/json</span></div>
  <pre>{hl_code}</pre>
</div>""")
            elif lang == 'http':
                hl_code = highlight_http(raw_code)
                code_title = f"{sec_num} HTTP Protocol" if sec_num else "HTTP Protocol"
                out_html.append(f"""<div class="code-box">
  <div class="code-top"><span style="color:#38bdf8;">{code_title}</span><span>HTTP/1.1</span></div>
  <pre>{hl_code}</pre>
</div>""")
            elif lang == 'html':
  ...
  <pre>{hl_code}</pre>
            ...
            elif lang in ('javascript', 'js') or (not lang and any(k in raw_code for k in ['fetch(', 'async ', 'await ', 'const ', 'function', 'console.log'])) or any(lang == m for m in ['urlsearchparams', 'response', 'request', 'bearer', 'abortcontroller', 'fetch', 'blob', 'options']):
                hl_code = highlight_js(raw_code)
                label = f"{last_subheading}" if (last_subheading and ".js" in last_subheading) else "JavaScript Fetch API"
...
# Split sections
splits = re.split(r'\n(?=#\s+(?:.*?\b22\.\d+\b|🧠\s+Quick|📝\s+Practice|🎯\s+Chapter))', raw_md)

opening_md = splits[0]
numbered_secs = splits[1:70] # 22.1 to 22.69 (69 sections)
must_know_md = splits[70]    # 22.70
quick_cheat_md = splits[71]  # Quick Cheat Sheet
practice_set_md = splits[72] # Practice Set
final_mental_md = splits[73] # 🎯 Chapter 22 Final Mental Map

# Part definitions: (trigger_sec_num, banner_text)
PART_TRIGGERS = {
    1: "Part 01 — Networking Protocols &amp; HTTP Client-Server Architecture (22.1 – 22.12)",
    13: "Part 02 — REST API Architecture, Resource URLs &amp; Status Codes (22.13 – 22.25)",
    26: "Part 03 — Request/Response Headers, Payloads &amp; Stream Aborts (22.26 – 22.38)",
    39: "Part 04 — Web Security: CORS, Preflights, Auth &amp; UI States (22.39 – 22.50)",
    51: "Part 05 — Enterprise Patterns: Pagination, DevTools &amp; Full CRUD (22.51 – 22.69)",
}
...
<head>
  ...
  <title>JS Master Study Documentation — Chapter 22: HTTP, REST API &amp; Fetch API</title>
...
<body>
  ...
  <div class="doc-page">
    ...
    <header class="master-banner">
      ...
      <div class="banner-sub">CHAPTER 22: HTTP, REST API &amp; FETCH API — CLIENT-SERVER NETWORKING &amp; FULL-STACK INTEGRATION</div>
      <div class="meta-grid">
        <div class="meta-item">
          <div class="meta-label">Total Sections</div>
          <div class="meta-val">70 Modules + Practice</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Curriculum Part</div>
          <div class="meta-val">Part 16 — Network Protocols &amp; Web APIs</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Core Mechanics</div>
          <div class="meta-val">HTTP, REST, JSON, Fetch API &amp; CRUD</div>
        </div>
        <div class="meta-item">
          <div class="meta-label">Reference Standard</div>
          <div class="meta-val">RFC 9110 / WHATWG Fetch / REST Architecture</div>
        </div>
      </div>
    </header>
    ...
    <div class="study-card" style="border-left: 4px solid var(--blue-accent); background: #f8fafc; padding: 8px 12px; margin-bottom: 6px;">
      <h2 style="font-size: 12px; color: var(--navy-mid); margin-bottom: 3px;">HTTP, REST API এবং Fetch API — Frontend থেকে Backend-এর সাথে সংযোগ</h2>
      <p class="text-p" style="font-size: 10px; margin-bottom: 3px;">Chapter 21-এ আমরা Asynchronous JavaScript ও Promise শিখেছি। একজন Full Stack ইঞ্জিনিয়ার হিসেবে ক্লায়েন্ট ব্রাউজার কীভাবে সার্ভারের সাথে যোগাযোগ করে, ডাটা আনা-নেওয়া করে এবং ব্যাকএন্ড ডাটাবেজে প্রভাব ফেলে—সেটি গভীরভাবে জানা অপরিহার্য।</p>
      <div class="def-box" style="margin: 2px 0; padding: 4px 6px;">
        <div class="def-text" style="font-size: 9px;">Browser Client &nbsp;→&nbsp; HTTP Request (Method, URL, Headers, Body) &nbsp;→&nbsp; Backend REST API &nbsp;→&nbsp; HTTP Response (Status, JSON)</div>
      </div>
      <p class="text-p" style="font-weight: 600; color: #0369a1; margin-top: 2px; font-size: 9.5px;">React, Next.js, Node.js বা Express—যেকোনো ওয়েব প্ল্যাটফর্মে ফুল-স্ট্যাক ডেভলপমেন্টের মূল মেরুদণ্ড হলো এই নেটওয়ার্কিং জ্ঞান। 🔥</p>
    </div>
""")

# Render all 69 numbered sections (22.1 to 22.69)
for idx, sec_text in enumerate(numbered_secs, start=1):
    ...
    m = re.search(r'\b(22\.\d+)\b\s*(.*)$', header_line)
    if m:
        sec_num, sec_title = m.group(1), m.group(2)
        sec_title = sec_title.strip('`* ')
    else:
        sec_num, sec_title = f"22.{idx}", header_line.lstrip('#').strip()

    card_style = ""
    badge_style = ""
    if "⚠️" in header_line or "Error" in sec_title or "CORS" in sec_title or "404" in sec_title or "ভুল" in sec_title:
        card_style = ' style="border-left: 4px solid #ef4444; background: #ffffff;"'
        badge_style = ' style="background: #dc2626;"'
    elif "CRUD" in sec_title or "Flow" in sec_title or "vs" in sec_title or "Structure" in sec_title or "Debugging" in sec_title or "Example" in sec_title:
        card_style = ' style="border-left: 4px solid #0284c7; background: #ffffff;"'
        badge_style = ' style="background: #0284c7;"'

    # Strict Page 1 Balance: Section 22.1 Card must break cleanly before Section 22.2
    if idx == 1:
        if card_style:
...
html_parts.append('    <div class="part-banner">Part 06 — REST API Mastery: Must Know, Cheat Sheet, Practice Lab &amp; Mental Map</div>\n')

# 1. Must Know (22.70)
must_know_body = parse_section_body(re.sub(r'^#\s+🔥\s+22\.70\s+Must Know', '', must_know_md).strip())
html_parts.append(f"""    <!-- 22.70 Must Know -->
    <div class="study-card" style="border-left: 4px solid #ef4444; background: #ffffff;">
      <div class="card-title"><span class="badge-num" style="background: #dc2626;">MUST KNOW</span> 🔥 22.70 Must Know — HTTP &amp; REST API Pillars</div>
{must_know_body}
    </div>
""")
...
html_parts.append(f"""    <!-- Quick Cheat Sheet -->
    <div class="study-card" style="border-left: 4px solid #10b981; background: #f0fdf4;">
      <div class="card-title"><span class="badge-num" style="background: #059669;">CHEAT SHEET</span> 🧠 Quick Cheat Sheet — HTTP Methods, Headers &amp; Fetch Patterns</div>
{cheat_body}
    </div>
""")

# 3. Practice Set (7 projects)
html_parts.append("""    <!-- Practice Set -->
    <div class="study-card" style="border-left: 4px solid #0284c7;">
      <div class="card-title"><span class="badge-num">PRACTICE LAB</span> 📝 Practice Set — 7 Practical API &amp; CRUD Challenges</div>
""")
...
summary_text = re.sub(r'^#\s+🎯\s+Chapter 22 Final Mental Map', '', final_mental_md).strip()
...
html_parts.append(f"""    <!-- Final Mental Map -->
    <div class="study-card" style="border-left: 4px solid #8b5cf6; background: #faf5ff;">
      <div class="card-title"><span class="badge-num" style="background: #7c3aed;">SUMMARY</span> 🎯 Chapter 22 Final Mental Map — Web API, HTTP &amp; Data Pipeline</div>
{rendered_summary}
    </div>
""")
...
html_parts.append("""    <!-- Next Chapter Preview -->
    <div class="study-card" style="border-left: 4px solid #0284c7; background: #f0f9ff;">
      <div class="card-title"><span class="badge-num">NEXT CHAPTER</span> 📘 Chapter 23 Preview — Advanced Regular Expressions (RegExp)</div>
      <p class="text-p" style="font-size: 12px; line-height: 1.6; color: #0c4a6e;">
        পরবর্তী অধ্যায়ে আমরা জাভাস্ক্রিপ্টের অন্যতম শক্তিশালী টেক্সট প্রসেসিং ইঞ্জিন <strong>Advanced Regular Expressions (RegExp)</strong> নিয়ে বিস্তারিত শিখব—Lookaheads, Lookbehinds, Named Capturing Groups, Unicode Property Escapes, এবং বাস্তব ডেটা ভ্যালিডেশন প্যাটার্নস।
      </p>
    </div>
""")
...
# Write HTML file
target_file = r'Code/Chapter-22-HTTP-API-Fetch.html'
